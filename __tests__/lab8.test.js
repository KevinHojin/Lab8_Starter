describe("Basic user flow for Website", () => {
  ...
  it("Make sure <product-item> elements are populated", async () => {
    ...
    */
    ...
    console.log(`Checking product items/${prodItems.length}`);
    // Grab the .data property of <product-items> to grab all of the json data stored inside
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowConst = await prodItem.getProperty("shadowRoot");
    const newShadow = shadowConst.asElement();
    const getButton = await newShadow.$("button");
    await getButton.click();
    const text = await getButton.getProperty("innerText");
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      const shadowConst = await prodItems[i].getProperty("shadowRoot");
      const newShadow = shadowConst.asElement();
      const getButton = await newShadow.$("button");
      await getButton.click();
    }
    const count = await page.$("#cart-count");
    const outputCount = await count.getProperty("innerText");
    expect(outputCount["_remoteObject"].value).toBe("20");
  }, 10000);
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (i = 0; i < prodItems.length; i++) {
      const shadowConst = await prodItems[i].getProperty("shadowRoot");
      const newShadow = shadowConst.asElement();
      getButton = await newShadow.$("button");
      const text = await getButton.getProperty("innerText");
      expect(text["_remoteObject"].value).toBe("Remove from Cart");
    }
    //const text = await getButton.getProperty("innerText");
  ...
  it("Checking the localStorage to make sure cart is correct", async () => {
    ...
    //const storage = await page.$("window");
    const windowHandle = await page.$("window");
    const items = await page.evaluate(
      (e) => window.localStorage.getItem("cart"),
      windowHandle
    );
  ...
  it("Checking number of items in cart on screen after removing from cart", async () => {
    ...
    const count = await page.$("#cart-count");
    const outputCount = await count.getProperty("innerText");
    expect(outputCount["_remoteObject"].value).toBe("0");
  }, 10000);
